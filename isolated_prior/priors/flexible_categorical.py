...
class MulticlassRank(nn.Module):
    def __init__(self, num_classes, ordered_p=0.5):
        super().__init__()
        # TODO: Uncertain if this is correct, should be a sampler
...
class FlexibleCategorical(torch.nn.Module):
    ...
    def forward(self, batch_size):
        ...
        # Generate edge index
        edge_index = generate_edge_index(x, y, self.h, self.args['device'])
        ...
        return x, y, y, edge_index  # x.shape = (T,B,H)
...
def get_batch(batch_size, seq_len, num_features, get_batch, device, hyperparameters=None, batch_size_per_gp_sample=None, **kwargs):
    ...
    # Sixth call to get_batch - belongs to the FlexibleCategorical class
    sample = [model(batch_size=batch_size_per_gp_sample) for model in models]

    x, y, y_, edge_index = zip(*sample)
    x, y, y_, edge_index = torch.cat(x, 1).detach(), torch.cat(y, 1).detach(), torch.cat(y_, 1).detach(), torch.cat(edge_index, 1).detach()
