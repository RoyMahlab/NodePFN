...
def get_general_config(max_features, bptt, eval_positions=None):
    ...
    config_general = {
        ...
        "eval_positions": None,
        "seq_len_used": bptt,
        "sampling": 'mixed',#hp.choice('sampling', ['mixed', 'normal']), # uniform
        "epochs": 50,
        "num_steps": 100,
        "verbose": False,
...
def get_network_config():
    """
    Return network generation hyperparameters.
    """
    config_network = {
        "graph_type": CSH.CategoricalHyperparameter('graph_type', ['sbm', 'random']),
        "homophily_rate": CSH.UniformFloatHyperparameter('homophily_rate', lower=0.1, upper=0.9),
        "p_in": CSH.UniformFloatHyperparameter('p_in', lower=0.01, upper=0.1),  # Probability of an edge within a community
        "edge_prob": CSH.UniformFloatHyperparameter('edge_prob', lower=0.01, upper=0.05),  # Probability of an edge within a community
    }
    return config_network

def get_diff_network():
    """
    Return network generation hyperparameters for differentiable wrapper.
    """
    diff_network = {
        "graph_type": {'distribution': 'meta_choice', 'choice_values': ['sbm', 'random']},
        "homophily_rate": {'distribution': 'uniform', 'min': 0.1, 'max': 0.9,},
        "p_in": {'distribution': 'uniform', 'min': 0.01, 'max': 0.1},
        "edge_prob": {'distribution': 'uniform', 'min': 0.01, 'max': 0.05 },  # Probability of an edge within a community
    }
    return diff_network
...
def get_diff_config():
    ...
    diff_network = get_diff_network()

    config_diff = {'differentiable_hyperparameters': {**diff_prior_bag, **diff_causal, **diff_gp, **diff_flex, **diff_network}}
...
def get_prior_config_causal(max_features=100):
    ...
    config_network = get_network_config()
