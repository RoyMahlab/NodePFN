...
def load_model_only_inference(path, filename, device):
    ...
    """
    logger.info(f"Loading model from {os.path.join(path, filename)}")
    ...
    try:
        if config_sample['pos_encoder'] == 'none' or config_sample['pos_encoder'] is None:
            pos_encoder_generator = None
        elif config_sample['pos_encoder'] == 'lap_pe':
            pos_encoder_generator = positional_encodings.LapPePositionalEncodings(config_sample['emsize'], config_sample['bptt']*2)
    except: 
        pos_encoder_generator = None
...
def get_meta_gp_prior_hyperparameters(config):
    from nodepfn.priors.utils import trunc_norm_sampler_f
    config = {hp: (list(config[hp].values())[0]) if type(config[hp]) is dict else config[hp] for hp in config}
