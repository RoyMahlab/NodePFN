...
def load_model_workflow(i, e, add_name, base_path, device='cpu', eval_addition='', only_inference=True):
    ...
    def get_file(e):
        ...
        """
        model_file = f'checkpoint_epoch_{e}.ckpt'
        model_path = os.path.join(base_path, model_file)
        results_file = model_path
        results_file = os.path.join(base_path,
                                    f'models_ckpts/{add_name}/epoch_{e}_{eval_addition}.pkl')
    ...
    def check_file(e):
        ...
        if not Path(model_path).is_file():  # or Path(results_file).is_file():
            print('We have to download the NodePFN, as there is no checkpoint at ', model_path)
            import requests
            url = 'https://github.com/jeongwhanchoi/NodePFN/raw/refs/tags/v1.0.0/nodepfn/models_ckpts/checkpoint_epoch_30.ckpt'
    ...
    model_file = None
    if e == -1:
        for e_ in [30] + list(range(100, -1, -1)):
...
class NodePFNClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, device='cpu', base_path=pathlib.Path(__file__).parent.parent.resolve(), model_string='',
        ...
        :param model_string: Name of the model. Used first to check if the model is already in memory, and if not, 
               ...
               follows: "prior_diff_real_checkpoint" + model_string + "_n_0_epoch_e.ckpt", where e can be a number 
    ...
    def fit(self, X, y, edge_index, overwrite_warning=False):
        """
        ...
        if X.shape[0] > 1024 and not overwrite_warning:
            raise ValueError("⚠️ WARNING: NodePFN is not made for datasets with a trainingsize > 1024. Prediction might take a while, be less reliable. We advise not to run datasets > 10k samples, which might lead to your machine crashing (due to quadratic memory scaling of NodePFN). Please confirm you want to run by passing overwrite_warning=True to the fit function.")
...
def transformer_predict(model, eval_xs, eval_ys, eval_position, edge_index,
                        device='cpu',
                        max_features=1000,
                        style=None,
