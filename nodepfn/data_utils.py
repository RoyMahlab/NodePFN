...
def class_rand_splits(label, label_num_per_class, valid_num=500, test_num=1000):
    ...
    n_non = non_train_idx.shape[0]

    valid_count = int(valid_num)

    test_count = int(test_num)
    valid_count = min(valid_count, n_non)
    test_count = min(test_count, max(0, n_non - valid_count))
    valid_idx = non_train_idx[:valid_count]
    test_idx = non_train_idx[valid_count : valid_count + test_count]
    print(f"train:{train_idx.shape}, valid:{valid_idx.shape}, test:{test_idx.shape}")
    return {"train": train_idx, "valid": valid_idx, "test": test_idx}

def class_rand_splits_half(label, label_num_per_class):
    ...
    half = non_train_idx.shape[0] // 2
    valid_idx = non_train_idx[perm[:half]]
    test_idx = non_train_idx[perm[half:]]
    print(f"train:{train_idx.shape}, valid:{valid_idx.shape}, test:{test_idx.shape}")
    split_idx = {"train": train_idx, "valid": valid_idx, "test": test_idx}
    return split_idx

def load_fixed_splits(data_dir, dataset, name, label_num_per_class):
    splits_lst = []
    if name in ['roman-empire', 'amazon-ratings', 'minesweeper', 'tolokers', 'questions']:
        ...
        torch_dataset = Reddit(root=data_dir)
        data = torch_dataset[0]
        splits = {}
        ...
        for i in range(10):
            ...
            splits_lst.append(splits)
    elif name in ["actor", "cornell", "texas"]:
        file_path = f"{data_dir}/geom-gcn/{name}/{name}.npz"
        data = np.load(file_path)
        train_masks = data["train_masks"]  # (10, N), 10 splits
        val_masks = data["val_masks"]
        test_masks = data["test_masks"]
        ...
        splits = class_rand_splits(dataset.label, label_num_per_class=label_num_per_class, valid_num=500, test_num=1000)
        splits_lst.append(splits)
    else :
        splits = class_rand_splits_half(dataset.label, label_num_per_class=label_num_per_class)
        splits_lst.append(splits)
