...
class TransformerModel(nn.Module):
    def __init__(self, encoder, n_out, ninp, nhead, nhid, nlayers, dropout=0.0, style_encoder=None, y_encoder=None,
        ...
        self.k_neighbor_sampling = k_neighbor_sampling
    ...
    def forward(self, src, src_mask=None, single_eval_pos=None):
        ...
        if self.pos_encoder is not None:
            if 'edge_index' in self.pos_encoder.forward.__code__.co_varnames:
                src = self.pos_encoder(src, edge_index=edge_index)
            else:
...
class TransformerEncoderDiffInit(Module):
    ...
    def forward(self, src: Tensor, mask: Optional[Tensor] = None, src_key_padding_mask: Optional[Tensor] = None, 
        ...
        Args:
            ...
            edge_index: edge indices for local GNN (optional).
